...
num_gpus = 8
batch_size = 2
num_iters_per_epoch = 34149 // (num_gpus * batch_size)
num_epochs = 50
...
train_pipeline = [
    ...
    dict(type='Collect3D', keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'gt_bboxes', 'gt_labels', 'centers2d', 'depths', 'prev_exists'] + collect_keys,
         meta_keys=('filename', 'ori_shape', 'img_shape', 'pad_shape', 'scale_factor', 'flip', 'box_mode_3d', 'box_type_3d',
                    'img_norm_cfg', 'scene_token', 'gt_bboxes_3d', 'gt_labels_3d', 'results_plane', 'intrinsics', 'extrinsics',
                    'proposals_intrinsics'))
]
test_pipeline = [
    dict(type='LoadMultiViewImageFromFiles', to_float32=True),
    dict(type='ResizeCropFlipRotImage',
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=[data_root + 'nuscenes2d_temporal_infos_train_stream.pkl',
                  data_root + 'nuscenes2d_temporal_infos_val_stream.pkl'],
        num_frame_losses=num_frame_losses,
    ...
    val=dict(type=dataset_type, pipeline=test_pipeline, collect_keys=collect_keys + \
             ['img', 'img_metas'], queue_length=queue_length, ann_file=data_root + 'nuscenes2d_temporal_infos_val_stream.pkl', classes=class_names, modality=input_modality),
    test=dict(type=dataset_type, pipeline=test_pipeline, collect_keys=collect_keys + \
              ['img', 'img_metas'], queue_length=queue_length, ann_file=data_root + 'nuscenes2d_temporal_infos_test_stream.pkl', classes=class_names, modality=input_modality),
...
load_from = '/opt/data/private/jihao/Project/StreamPETR-main/ckpts/dd3d_det_final.pth'
resume_from = None
