...
class CustomNuScenesDataset(NuScenesDataset):
    ...
    def __init__(self, collect_keys, seq_mode=False, seq_split_num=1, num_frame_losses=1, queue_length=8, random_length=0, *args, **kwargs):
        ...
        results_xyxy_path = 'projects/mmdet3d_plugin/datasets/results_save_total.pkl'
        with open(results_xyxy_path, 'rb') as fr:
            results_xyxy = pickle.load(fr)
        self.results_xyxy = results_xyxy
        ...
        if seq_mode:
            ...
            # Must be called after load_annotations b/c load_annotations does sorting.
    ...
    def _set_sequence_group_flag(self):
        ...
        if self.seq_split_num != 1:
            if self.seq_split_num == 'all':
                ...
                assert len(np.bincount(new_flags)) == len(
                    np.bincount(self.flag)) * self.seq_split_num
    ...
    def prepare_train_data(self, index):
        ...
        """
        queue = []
        index_list = list(range(index-self.queue_length -
                          self.random_length+1, index))
    ...
    def union2one(self, queue):
        for key in self.collect_keys:
            if key != 'img_metas':
                queue[-1][key] = DC(torch.stack([each[key].data for each in queue]),
    ...
    def get_data_info(self, index):
        ...
        e2g_matrix = convert_egopose_to_matrix_numpy(
            e2g_rotation, e2g_translation)
        l2e_matrix = convert_egopose_to_matrix_numpy(
            l2e_rotation, l2e_translation)
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                cam2lidar_rt = convert_egopose_to_matrix_numpy(
                ...
                # obtain 2d detection results
                img_name = cam_info['data_path'].split('/')[-1]
                if img_name in self.results_xyxy:
                    objects_results = self.results_xyxy[img_name]
                else:
                    objects_results = []
                results_totals.append(objects_results)
            ...
            input_dict.update(
                dict(
                    ...
                    results_plane=results_totals,
                    proposals_intrinsics=deepcopy(intrinsics),
                ))
        if not self.test_mode:
