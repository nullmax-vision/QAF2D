...
class FocalHead(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        self.shared_reg = nn.Sequential(
            nn.Conv2d(self.in_channels, self.embed_dims,
    ...
    def forward(self, location, **data):
        ...
        cls_logits = cls.permute(0, 2, 3, 1).reshape(
            bs*n, -1, self.num_classes)
        ...
        ltrb = ltrb.sigmoid()
        centers2d_offset = self.center2d(
            reg_feat).permute(0, 2, 3, 1).contiguous()
        ...
        sample_weight = cls_score.detach().sigmoid(
        ) * centerness.detach().view(bs, -1, 1).sigmoid()
    ...
    def loss(self,
        ...
        all_gt_bboxes2d_list = [
            bboxes2d for i in gt_bboxes2d_list for bboxes2d in i]
        all_gt_labels2d_list = [
            labels2d for i in gt_labels2d_list for labels2d in i]
    ...
    def loss_single(self,
        ...
        iou_score = bbox_overlaps(
            bboxes_gt, bboxes, is_aligned=True).reshape(-1)
        ...
        (heatmaps, ) = multi_apply(self._get_heatmap_single,
                                   all_centers2d_list, gt_bboxes_list, img_shape)
    ...
    def _get_heatmap_single(self, obj_centers2d, obj_bboxes, img_shape):
        ...
        heatmap = torch.zeros(img_h // self.stride, img_w //
                              self.stride, device=obj_centers2d.device)
        if len(obj_centers2d) != 0:
            ...
            for center, r in zip(obj_centers2d, radius):
                heatmap = draw_heatmap_gaussian(
                    heatmap, center / 16, radius=int(r), k=1)
        return (heatmap, )
    ...
    def _get_target_single(self,
        ...
        # centers2d target
        centers2d_targets = bbox_pred.new_full(
            (num_bboxes, 2), 0.0, dtype=torch.float32)
