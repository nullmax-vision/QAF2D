...
from projects.mmdet3d_plugin.models.utils.positional_encoding import pos2posemb3d, pos2posemb1d, nerf_positional_encoding, gen_sineembed_for_position
...
class PerspectiveHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is PerspectiveHead):
        ...
        super(PerspectiveHead, self).__init__(
    ...
    def _init_layers(self):
        ...
        self.boxs_embeds = nn.Embedding(self.num_query, 3)
        # self.tgt_embeds = nn.Embedding(self.num_query, self.embed_dims)
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        temp_pos = self.transformer.decoder.query_embedding(
            pos2posemb3d(temp_reference_point))
    ...
    def prepare_for_dn(self, batch_size, reference_points, img_metas, num_proposals=0):
        if self.training and self.with_dn:
            ...
            known_labels = labels.repeat(
                self.scalar, 1).view(-1).long().to(reference_points.device)
            known_bid = batch_idx.repeat(self.scalar, 1).view(-1)
            known_bboxs = boxes.repeat(self.scalar, 1).to(
                reference_points.device)
            known_bbox_center = known_bboxs[:, :3].clone()
            known_bbox_scale = known_bboxs[:, 3:6].clone()
            known_bbox_yaw = known_bboxs[:, 6:7].clone()

            if self.bbox_noise_scale > 0:
                diff_center = known_bbox_scale / 2 + self.bbox_noise_trans
                diff_scale = known_bbox_scale
                diff_raw = torch.ones_like(known_bbox_yaw) * math.pi / 6

                # noising bbox center
                rand_prob = torch.rand_like(known_bbox_center) * 2 - 1.0
                known_bbox_center += torch.mul(rand_prob,
                                               diff_center) * self.bbox_noise_scale
                known_bbox_center[..., 0:3] = (
                    known_bbox_center[..., 0:3] - self.pc_range[0:3]) / \
                    (self.pc_range[3:6] - self.pc_range[0:3])

                # noising bbox scale
                rand_prob_scale = torch.rand_like(
                    known_bbox_scale) * 2 - 1.0
                known_bbox_scale[..., 0:3] += torch.mul(rand_prob_scale,
                                                        diff_scale) * self.bbox_noise_scale
                # noising bbox raw
                rand_prob_yaw = torch.rand_like(
                    known_bbox_yaw) * 2 - 1.0
                known_bbox_yaw[..., 0:1] += torch.mul(
                    rand_prob_yaw, diff_raw) * self.bbox_noise_scale
            ...
            padding_bbox = torch.zeros(batch_size, pad_size, 3).to(
                reference_points.device)
            padded_reference_points = torch.cat(
                [padding_bbox, reference_points], dim=1)  # .unsqueeze(0).repeat(batch_size, 1, 1)

            if len(known_num):
                map_known_indice = torch.cat(
                    [torch.tensor(range(num)) for num in known_num])  # [1,2, 1,2,3]
                map_known_indice = torch.cat(
                    [map_known_indice + single_pad * i for i in range(self.scalar)]).long()
            if len(known_bid):
                padded_boxs_info = padded_reference_points[(
                    known_bid.long(), map_known_indice)].clone()
                padded_boxs_info[..., :3] = known_bbox_center.to(
                    reference_points.device)
                if False:
                    padded_boxs_info[..., 3:6] = known_bbox_scale.to(
                        reference_points.device)
                    padded_boxs_info[..., 6:7] = known_bbox_yaw.sin().to(
                        reference_points.device)
                    padded_boxs_info[..., 7:8] = known_bbox_yaw.cos().to(
                        reference_points.device)
                padded_reference_points[(
                    known_bid.long(), map_known_indice)] = padded_boxs_info.clone()
            ...
            query_size = pad_size + num_proposals + self.num_query + self.num_propagated
            tgt_size = pad_size + num_proposals + self.num_query + self.memory_len
            temporal_attn_mask = torch.ones(
                query_size, tgt_size).to(reference_points.device) < 0
            temporal_attn_mask[:attn_mask.size(
                0), :attn_mask.size(1)] = attn_mask
    ...
    def init_weights(self):
        ...
        nn.init.uniform_(self.boxs_embeds.weight.data, 0, 1)
        if self.num_propagated > 0:
    ...
    def pre_update_memory(self, data):
        ...
        if self.memory_embedding is None:
            self.memory_embedding = x.new_zeros(
            ...
            self.memory_egopose = data['ego_pose_inv'].unsqueeze(
                1) @ self.memory_egopose
            ...
            self.memory_velo = memory_refresh(
                self.memory_velo[:, :self.memory_len], x)
        ...
        if self.num_propagated > 0:
            ...
            self.memory_reference_point[:, :self.num_propagated] = self.memory_reference_point[:,
                                                                                               :self.num_propagated] + (1 - x).view(B, 1, 1) * pseudo_reference_points
            self.memory_egopose[:, :self.num_propagated] = self.memory_egopose[:,
                                                                               :self.num_propagated] + (1 - x).view(B, 1, 1, 1) * torch.eye(4, device=x.device)
    ...
    def post_update_memory(self, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict):
        ...
        rec_reference_points = topk_gather(
            rec_reference_points, topk_indexes).detach()
        ...
        self.memory_timestamp -= data['timestamp'].unsqueeze(-1).unsqueeze(-1)
        self.memory_egopose = data['ego_pose'].unsqueeze(
            1) @ self.memory_egopose

    def proposals_flatten(self, init_proposals):

        proposals_3d = list()
        for temp in range(len(init_proposals)):
            temp_init_proposals = init_proposals[temp]
            temp_proposals_8d = list()
            max_num = 0
            for idx, num_batch in enumerate(range(len(temp_init_proposals))):
                batch_proposals_8d = list()
                proposals_ = temp_init_proposals[num_batch]
                for cam_proposal in proposals_:
                    batch_proposals_8d += [p[-1] for p in cam_proposal]
                if max_num <= len(batch_proposals_8d):
                    max_num = len(batch_proposals_8d)
                if len(batch_proposals_8d) == 0:
                    batch_proposals_8d = [torch.zeros((8)).to(
                        self.boxs_embeds.weight.device)]
                temp_proposals_8d.append(
                    torch.stack(batch_proposals_8d, dim=0))
            batch_size = len(temp_proposals_8d)
            padded_reference_pts = torch.zeros(
                (batch_size, max_num, 8)).to(self.boxs_embeds.weight.device)
            for batch_idx in range(batch_size):
                pts8d = temp_proposals_8d[batch_idx]
                padded_reference_pts[batch_idx,
                                     :pts8d.size(0), ...] = pts8d
            proposals_3d.append(padded_reference_pts)
        proposals_3d = torch.stack(proposals_3d, dim=0).permute(1, 0, 2, 3)

        return proposals_3d

    def forward(self, img_metas, init_proposals=None, **data):
        """Forward function.
        Args:
        ...
        # proposals reference points 8d
        proposals_reference_pts = self.proposals_flatten(
            init_proposals).squeeze(1)
        num_proposals = proposals_reference_pts.size(1)

        proposals_reference_pts[..., 0:3] = (
            proposals_reference_pts[..., 0:3] - self.pc_range[0:3]) / \
            (self.pc_range[3:6] - self.pc_range[0:3])
        proposals_reference_pts = proposals_reference_pts.clamp(
            min=0.0, max=1.0)

        # (644, 3): (num_queries, 3)
        # 首先将reference poins设置为8d的点，随后再将8d的点进行位置编码
        # 随后将8d的点进行编码

        reference_points_8d = self.boxs_embeds.weight[None].repeat(
            B, 1, 1)

        if False:
            ref_sin = reference_points_8d[..., -2].clone().data
            ref_cos = reference_points_8d[..., -1].clone().data
            norm_sc = torch.ones_like(reference_points_8d)
            norm_sc[..., -2] = 1. / (ref_sin.pow(2) + ref_cos.pow(2)).sqrt()
            norm_sc[..., -1] = 1. / (ref_sin.pow(2) + ref_cos.pow(2)).sqrt()
            reference_points_8d = norm_sc * reference_points_8d
        reference_points_8d = torch.cat(
            (proposals_reference_pts[..., :3], reference_points_8d), dim=1)
        reference_points = reference_points_8d[..., :3]
        # reference_points = reference_points.sigmoid()

        dtype = reference_points_8d.dtype
        intrinsics = data['intrinsics'] / 1e3
        extrinsics = data['extrinsics'][..., :3, :]
        mln_input = torch.cat(
            [intrinsics[..., 0, 0:1], intrinsics[..., 1, 1:2], extrinsics.flatten(-2)], dim=-1)
        ...
        # memory; B*N, num_memory, dim (like bev)
        feat_flatten = torch.cat(feat_flatten, dim=1)
        spatial_flatten = torch.as_tensor(
            spatial_flatten, dtype=torch.long, device=mlvl_feats[0].device)
        level_start_index = torch.cat((spatial_flatten.new_zeros(
            (1, )), spatial_flatten.prod(1).cumsum(0)[:-1]))

        # bs, (330+num_queries), 3  reference_points 的坐标，表示3d bboxs的中心点
        reference_points, attn_mask, mask_dict = self.prepare_for_dn(
            B, reference_points, img_metas, num_proposals=num_proposals)

        # bs, (330+num_queries), embed_dim
        query_pos = self.transformer.decoder.query_embedding(
        ...
        outs_dec, inter_reference_pts = self.transformer(tgt, query_pos, feat_flatten, spatial_flatten, level_start_index, temp_memory,
            ...
            if False:
                if lvl == 0:
                    ...
                    reference = inverse_sigmoid(
                        inter_reference_pts[lvl-1].clone())
    ...
    def _get_target_single_proposals(self,
                                     cls_score,
                                     bbox_pred,
                                     cls_2ds,
                                     gt_labels,
        ...
        cls_inds = list(set(gt_labels.detach().cpu().tolist()))
        ...
        for cls_ind in cls_inds:
            matching_gt_idx = torch.nonzero(gt_labels == cls_ind)
            matching_pred_idx = torch.nonzero(cls_2ds == cls_ind)

            assign_result_cls = self.assigner.assign(bbox_pred[matching_pred_idx], cls_score[matching_pred_idx],
                                                     gt_bboxes[matching_gt_idx], gt_labels[matching_gt_idx],
                                                     gt_bboxes_ignore)

            sampling_result_cls = self.sampler.sample(assign_result_cls, bbox_pred,
                                                      gt_bboxes)
            pos_inds_cls = sampling_result_cls.pos_inds
            neg_inds_cls = sampling_result_cls.neg_inds
            pos_inds.append(pos_inds_cls)
            neg_inds.append(neg_inds_cls)
            pos_assigned_gt_inds.append(
                sampling_result_cls.pos_assigned_gt_inds)
            pos_gt_bboxes.append(sampling_result_cls.pos_gt_bboxes)

        pos_inds = torch.tensor(pos_inds).to(cls_score.device)
        neg_inds = torch.tensor(neg_inds).to(cls_score.device)
        pos_assigned_gt_inds = torch.tensor(
            pos_assigned_gt_inds).to(cls_score.device)
        pos_gt_bboxes = torch.stack(
            pos_gt_bboxes, dim=0).to(cls_score.device)
        ...
        # DETR
        bbox_targets[pos_inds] = pos_gt_bboxes  # sampling_result.pos_gt_bboxes
        return (labels, label_weights, bbox_targets, bbox_weights,
                pos_inds, neg_inds)
    ...
    def dn_loss_single(self,
        ...
        cls_avg_factor = num_total_pos * 3.14159 / 6 * \
            self.split * self.split * self.split  # positive rate
        if self.sync_cls_avg_factor:
            cls_avg_factor = reduce_mean(
                cls_scores.new_tensor([cls_avg_factor]))
