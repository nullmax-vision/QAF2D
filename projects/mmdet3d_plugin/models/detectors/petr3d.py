...
class Petr3D(MVXTwoStageDetector):
    ...
    def obtain_history_memory(self,
        ...
        for i in range(T):
            ...
            loss = self.forward_pts_train(gt_bboxes_3d[i],
                                          ...
                                          gt_labels[i], img_metas[i], centers2d[i],
                                          depths[i], requires_grad=requires_grad, return_losses=return_losses,
    ...
    def prepare_location(self, img_metas, **data):
        ...
        location = locations(x, self.stride, pad_h, pad_w)[
            None].repeat(bs*n, 1, 1, 1)
        return location
        ...
        if not requires_grad:
            ...
            with torch.no_grad():
                outs = self.pts_bbox_head(
                    location, img_metas, None, **data, init_proposals=init_proposals)
            self.train()

        else:
            outs_roi = self.forward_roi_head(location, **data)
            # 根据分类分数找出最突出的点，这个部分类似于对分割任务的训练过程，
            # 对每一个pixel进行一个分类预测，然后找出pixel-levek分类分数最高的topk作为top-index
            topk_indexes = outs_roi['topk_indexes']

            flops, params = profile(
                self.pts_bbox_head, inputs=(location, img_metas, topk_indexes, init_proposals, data,))
            print('FLOPs = ' + str(flops/(2*6*1000**3)) + 'G',
                  'Params = ' + str(params/(2*1000**2)) + 'M')

            outs = self.pts_bbox_head(
                location, img_metas, topk_indexes, **data, init_proposals=init_proposals)
        ...
        if return_losses:
            ...
            if self.with_img_roi_head:
                loss2d_inputs = [gt_bboxes, gt_labels,
                                 centers2d, depths, outs_roi, img_metas]
    ...
    def forward_train(self,
        ...
        rec_img_feats = self.extract_feat(
            rec_img, self.num_frame_backbone_grads)
        ...
        if T-self.num_frame_backbone_grads > 0:
            ...
            data['img_feats'] = torch.cat(
                [prev_img_feats, rec_img_feats], dim=1)
        else:
    ...
    def simple_test_pts(self, img_metas, init_proposals=None, **data):
        ...
        outs = self.pts_bbox_head(
            location, img_metas, topk_indexes, init_proposals=init_proposals, **data)
...
# /opt/data/private/jihao/Project/dab-streampetr-baseline/projects/configs/StreamPETR/stream_petr_r50_flash_704_bs2_seq_428q_nui_60e.py
