...
from projects.mmdet3d_plugin.models.utils.positional_encoding import pos2posemb3d, gen_sineembed_for_position
...
class Detr3DTransformer(BaseModule):
    ...
    def forward(self,
        ...
        inter_states, reference_points = self.decoder(
            query=query,
            query_pos=query_pos,
        ...
        return inter_states, reference_points
...
class Detr3DTransformerDecoder(TransformerLayerSequence):
    ...
    def __init__(self, embed_dims, *args, d_model=256, **kwargs):
...
class DeformableFeatureAggregationCuda(BaseModule):
    ...
    def forward(self, instance_feature, query_pos, feat_flatten, reference_points, spatial_flatten, level_start_index, pc_range, lidar2img_mat, img_metas):
        ...
        key_points = reference_points.unsqueeze(-2) + self.learnable_fc(
            instance_feature).reshape(bs, num_anchor, -1, 3)
    ...
    def _get_weights(self, instance_feature, anchor_embed, lidar2img_mat):
        ...
        weights = self.weights_fc(feat_pos).reshape(
            bs, num_anchor, -1, self.num_groups).softmax(dim=-2)
        weights = weights.reshape(
            bs, num_anchor, self.num_cams, -1, self.num_groups).permute(0, 2, 1, 4, 3).contiguous()
    ...
    def feature_sampling(self, feat_flatten, spatial_flatten, level_start_index, key_points, weights, lidar2img_mat, img_metas):
        ...
        points_2d = torch.matmul(
            lidar2img_mat[:, :, None, None], pts_extand[:, None, ..., None]).squeeze(-1)

        points_2d = points_2d[..., :2] / \
            torch.clamp(points_2d[..., 2:3], min=1e-5)
        points_2d[..., 0:1] = points_2d[..., 0:1] / \
            img_metas[0]['pad_shape'][0][1]
        points_2d[..., 1:2] = points_2d[..., 1:2] / \
            img_metas[0]['pad_shape'][0][0]
