...
def topk_gather(feat, topk_indexes):
    if topk_indexes is not None:
        ...
        feat = torch.gather(
            feat, 1, topk_indexes.repeat(1, 1, *feat_shape[2:]))
    return feat
...
def transform_reference_points(reference_points, egopose, reverse=False, translation=True):
    ...
    if not translation:
        matrix[..., :3, 3] = 0.0
    reference_points = (matrix.unsqueeze(
        1) @ reference_points.unsqueeze(-1)).squeeze(-1)[..., :3]
    return reference_points


def normlize_boxes(bboxes, cam2lidar=None):
    if cam2lidar is None:
        centers_embed = bboxes[..., :3]
        sizes_embed = bboxes[..., 3:6].log()
        thetas_ = bboxes[..., -1:]
        ...
        centers_embed = bboxes[..., :3]
        centers_masks = torch.ones_like(bboxes[..., :1])
        centers_embed = torch.cat(
            (centers_embed, centers_masks), dim=-1).float()
        centers_embed = centers_embed @ torch.from_numpy(
            cam2lidar).t().to(centers_embed.device)[..., :3]

        sizes_embed = bboxes[..., 3:6]
        thetas_ = bboxes[..., -1:]
        thetas_ = (-1 * thetas_ - np.pi / 2)
        ws_, hs_, ls_ = sizes_embed[...,
                                    0:1], sizes_embed[..., 1:2], sizes_embed[..., 2:3]

        ws_ = ws_.view(-1, 1)
        hs_ = hs_.view(-1, 1)
        ls_ = ls_.view(-1, 1)

        assert ws_.shape == hs_.shape and ls_.shape == hs_.shape
        theta_embed = torch.cat((thetas_.sin(), thetas_.cos()), dim=-1)
        sizes_embed = torch.cat((ws_, ls_, hs_), dim=-1).log()
        bboxs_3ds = torch.cat(
            (centers_embed, sizes_embed, theta_embed), dim=-1)
