    ...
    def forward(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        out = self.attn(
            ...
            k=key,
            v=value,
            key_padding_mask=None)[0]  # cross attn
...
class PETRTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self, query, *args, query_pos=None, reference_points=None, reg_branch=None,
                num_denoising=0, **kwargs):
        ...
        for lid, layer in enumerate(self.layers):
            output = layer(output, *args, query_pos=query_pos, **kwargs)

            if False:
                output = output.permute(1, 0, 2)
                if reg_branch is not None:
                    tmp = reg_branch[lid](output)

                new_reference_points = torch.zeros_like(reference_points)
                new_reference_points[..., :num_denoising,
                                     :3] = tmp[..., :num_denoising, :3]
                new_reference_points[..., num_denoising:, :3] = tmp[..., num_denoising:, :3] + \
                    inverse_sigmoid(reference_points)[..., num_denoising:, :3]
                new_reference_points = new_reference_points.sigmoid()  # .clone()
                reference_points = new_reference_points  # .detach()

                if lid + 1 <= len(self.layers):
...
class PETRTemporalTransformer(BaseModule):
    ...
    def forward(self, memory, tgt, query_pos, pos_embed, attn_masks, temp_memory=None,
                temp_pos=None, mask=None, reference_points=None,
                reg_branch=None, spatial_shapes=None, num_denoising=0):
        ...
        out_dec, inter_reference_points = self.decoder(
            query=tgt,
            key=memory,
            value=memory,
            key_pos=pos_embed,
            ...
            level_start_index=torch.tensor([0], device=query_pos.device),
            num_denoising=num_denoising
        )
        out_dec = out_dec.transpose(1, 2).contiguous()
        memory = memory.reshape(-1, bs, c).transpose(0, 1).contiguous()
        return out_dec, memory, inter_reference_points
    ...
    def _forward(self,
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                if temp_memory is not None:
                    temp_key = temp_value = torch.cat(
                        [query, temp_memory], dim=0)
